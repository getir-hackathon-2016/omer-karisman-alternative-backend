...
module.exports = function(app, config) {
    ...
    app.use('/login', function(req, res) {
        ...
        if (!username) {
            ...
            logger.info(msg);
            res.status(400).json({ message: msg, code: 410 });
            return;
        }
        ...
        if (!password) {
            ...
            res.status(400).json({ message: msg, code: 411 });
            return;
        }
            ...
            usersDb.collection(usersCollectionName, function (err, usersCollection) {
                ...
                usersCollection.findOne({ username: username }, function (err, user) {
                    if (err) {
                        ...
                        res.status(401).json({ message: 'User or password is invalid', code: 412 } );
                        return;
                    } 
                    else if( user.password !== password ) {
                        logger.info('Incorrect password for user ' + username);
                        res.status(401).json({ message: 'User or password is invalid', code: 413 } );
                        return;
                    }
